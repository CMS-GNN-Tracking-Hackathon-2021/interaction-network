...
parser.add_argument('--N', type=int, default=996,
...
graph_dir = '/home/lhv14/cmsgnn/output/PU200/2GeV/'
...
#        if (n_layers_hit > 1): 
    for particle in hits.particle_id.unique():
        ...
        for lc in layer_combos:
            if (lc==valid_layer_pairs).all(1).any():
                truth_edges += 1
#            layers = np.array(particle.layer_id.values)
            #print(layers)
