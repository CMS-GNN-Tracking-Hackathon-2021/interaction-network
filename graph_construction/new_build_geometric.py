...
#same_layer_pairs = np.stack([np.arange(1,n_det_layers,1), np.arange(1,n_det_layers,1)], axis=1)
#layer_pairs = np.concatenate((layer_pairs, same_layer_pairs), axis=0)
...
def select_segments(hits1, hits2, phi_slope_max, z0_max,
    ...
    #print(hit_pairs)
    #hit_pairs = hits1[keys].merge(hits2[keys], on='evt', suffixes=('_1', '_2'))
    #print(hit_pairs)
    #print(hit_pairs)
    #print('1 \n') 
...
def construct_graph(hits, layer_pairs, phi_slope_max, z0_max,
    ...
    layer1 = hits.layer_id.loc[segments.index_1].values
    layer2 = hits.layer_id.loc[segments.index_2].values
    true_layer1 = layer1[y==1]
    true_layer2 = layer2[y==1]
    true_pid1 = pid1[y==1]
    true_pid2 = pid2[y==1]
    true_z1 = hits.z.loc[segments.index_1].values[y==1]
    true_z2 = hits.z.loc[segments.index_2].values[y==1]
    pid_lookup = {}
    
    #print("hits: ", n_hits, "edges :", n_edges, "y :", y, "number true edges: ", sum(y), "length y", len(y))
...
def select_hits(hits, pt_min=0, endcaps=False):
...
def process_event(file, output_dir, pt_min, n_eta_sections, n_phi_sections,
    ...
    #print(data)
    data = data[['evt', 'hit_id', 'x', 'y', 'z', 'r', 'sim_pt', 'sim_eta', 'sim_phi', 'particle_id', 'volume_id', 'layer_id']]
    #calculate phi of hit 
    data['phi'] = np.arctan2(data.y, data.x)


    # extract the event number
    prefix = file.split("ntuple_PU200_event")[1].split('.h5')[0].strip()
    ...
    hits = select_hits(data, pt_min=pt_min, 
                       endcaps=endcaps).assign(evt=evt)
    #print(hits_sections)
...
def main():
    ...
    #file_prefixes = [prefix for prefix in file_prefixes
     #                if ((int(prefix.split("00000")[1]) >= args.start_evt) and
      #                   (int(prefix.split("00000")[1]) <= args.end_evt))]
