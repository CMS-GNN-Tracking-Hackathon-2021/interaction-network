...
#    n_det_layers = len(vlids)
 ...
 #   hits = pd.concat([vlid_groups.get_group(vlids[i]).assign(layer=i)
  #                    for i in range(n_det_layers)])
    ...
    #         .merge(particles[['particle_id', 'sim_pt', 'sim_eta']], on='particle_id'))
    # Calculate derived hits variables
    #r = np.sqrt(hits.x**2 + hits.y**2)
    #phi = np.arctan2(hits.y, hits.x)
    # Select the data columns we need
    #hits = (hits[['hit_id', 'z', 'layer_id']]
    #        .assign(r=r, phi=phi)
    #        .merge(truth[['hit_id', 'particle_id', 'sim_pt', 'sim_eta']], on='hit_id'))
    # Remove duplicate hits
    #hits = hits.loc[
    #    hits.groupby(['particle_id', 'layer_id'], as_index=False).r.idxmin()
    #]
...
def process_event(file, output_dir, pt_min, n_eta_sections, n_phi_sections,
    ...
    data = data[['evt', 'hit_id', 'x', 'y', 'z', 'r','sim_type', 'sim_id', 'sim_pt', 'sim_eta', 'sim_phi', 'particle_id', 'volume_id', 'layer_id']]
    ...
    try:
       ...
       # filenames = [os.path.join(output_dir, '%s_g%03i' % (base_prefix, i))
        ...
        filenames = [os.path.join(output_dir,'event%s_g%03i' % (prefix, i))
             for i in range(len(graphs))]
